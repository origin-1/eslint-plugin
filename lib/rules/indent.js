...
function isOpenBraceToken(token)
{
...
const meta =
{
    ...
    {
        indent:
        'Expected indentation of {{expected}} characters but found {{actual}}.',
        indentBlockComment:     'Add {{missing}} indentation spaces to this block comment.',
        unindentBlockComment:   'Remove {{extra}} indentation spaces from this block comment.',
    },
};
...
function create(context)
{
    ...
    {
        expectedIndents = Array(lineCount + 1).fill(0, 1);
    }
    ...
    function validateProgramExit(node)
    {
        ...
        {
            const expectedIndent = expectedIndents[line];
        ...
        expectedIndents = null;
    }

    function validateSwitchStatement(node)
    {
        const openingBrace =
        sourceCode.getTokenAfter(node.discriminant, { filter: isOpenBraceToken });
