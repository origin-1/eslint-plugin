...
const { bracketPairsIn, isOpeningParenToken, isTokenOnSameLine, makeRuleDocsURL } =
require('../utils');
...
function create(context)
{
    ...
    function validateIIFE(node)
    {
        ...
        const firstLine = calleeOpeningParen.loc.start.line;
        if (firstLine < node.loc.start.line)
            return; // The callee does not start on the same line as the wrapping parenthesis.
        const calleeClosingParen = sourceCode.getTokenAfter(node);
        const tokenAfterClosingParen = sourceCode.getTokenAfter(calleeClosingParen);
        if (!isOpeningParenToken(tokenAfterClosingParen))
            return; // The callee is apparently wrapped in additional parentheses.
        const lastLine = calleeClosingParen.loc.end.line;
        ...
        if (paramsClosingParen.loc.end.line === lastLine)
        {
            const paramsOpeningParen = tokenAfterClosingParen;
            validJunctionSet.add(paramsOpeningParen.range[0]);
            validJunctionSet.add(calleeClosingParen.range[1]);
        }
        const { body } = node;
        if (body.type === 'BlockStatement')
        {
            ...
            if
            (
                (openingBrace = sourceCode.getFirstToken(body)).loc.start.line === firstLine &&
                (closingBrace = sourceCode.getLastToken(body)).loc.end.line === lastLine
            )
            {
                validJunctionSet.add(openingBrace.range[0]);
                validJunctionSet.add(closingBrace.range[1]);
            }
        }
    }
