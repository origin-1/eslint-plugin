...
const DETACHED_PUNCTUATOR_SET = new Set('(.)[]{}');
...
module.exports =
{
    meta:
    {
        ...
        docs:
        {
            ...
            url:         'https://github.com/origin-1/eslint-plugin#bracket-layout',
        },
        fixable:    'whitespace',
        schema:     [],
        messages:
        {
            ...
            sameLineAfterOpen:
            'Code after opening bracket "{{bracket}}" should be on next line.',
            sameLineBeforeClose:
            'Closing bracket "{{bracket}}" should be on the same line as opening bracket or ' +
            'after the previous line of code.',
            sameLineAfterClose:
            'Code after closing bracket "{{bracket}}" should be on next line.',
        },
    },
    ...
    create(context)
    {
        ...
        function canBreakAfter(token)
        {
            let returnValue;
            {
                ...
                if (type === 'Identifier' && value === 'async')
        ...
        function canBreakBefore(token)
        {
            ...
            const { type, value } = token;
            if (type === 'Punctuator')
                returnValue = DETACHED_PUNCTUATOR_SET.has(token.value);
        ...
        function validateProgram(node)
        {
            ...
            {
                if (OPEN_BRACKET_SET.has(token.value))
                    stack.push(token);
                else
                {
        ...
        const result = { Program: validateProgram };
        return result;
    },
};
