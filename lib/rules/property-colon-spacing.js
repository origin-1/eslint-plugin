...
module.exports =
{
    meta:
    {
        ...
        {
            description:
            'Enforce consistent spacing around the colon in object literal properties',
            url:
            'https://github.com/origin-1/eslint-plugin#property-colon-spacing',
        },
        fixable:    'whitespace',
        schema:     [],
        messages:
        {
            ...
            'Extra space after colon of {{computed}}property \'{{key}}\'.',
            extraSpaceBeforeColon:
            'Extra space before colon of {{computed}}property \'{{key}}\'.',
            missingSpaceAfterColon:
            'Missing space after colon of {{computed}}property \'{{key}}\'.',
        },
    },
    ...
    create(context)
    {
        ...
        function getOverlappingProperties(properties)
        {
            ...
            {
                if (currProperty.loc.start.line === lastProperty?.loc.end.line)
                {
         ...
         * Properties that are explicitly excuded and those that do not start on the same line as
         * their first token after the colon will not be mapped.
         * If the returned `Map` object is not empty, it will have an additional property
         * `expectedShift` containing the shift that should be enforced by the rule for all first
         * tokens after the colon of the mapped properties.
        ...
        function verifySpacing(node)
        {
            ...
            for (const property of keyValueProperties)
            {
                ...
                if (spaceBeforeColon)
                {
                    ...
                    report
                    (property, colon, 'extraSpaceBeforeColon', beforeColonEnd, colonStart, fix);
                }
                if (colonEnd.line === afterColonStart.line)
                {
                    ...
                    else if (missingSpace < 0)
                    {
                        ...
                        const fix =
                        fixer => fixer.removeRange([endOffset + missingSpace, endOffset]);
                        report
                        (property, colon, 'extraSpaceAfterColon', colonEnd, afterColonStart, fix);
                    }
                }
            }
        }
        ...
        const result = { ObjectExpression: verifySpacing };
        return result;
    },
};
