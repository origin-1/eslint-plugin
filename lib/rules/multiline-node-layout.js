...
function create(context)
{
    ...
    function isHandledPunctuator(token)
    {
        ...
        if (type !== 'Punctuator') return false;
        if (value === '?')
        {
            // A "?" immediately followed by ":" marks an optional property or parameter.
            const nextToken = sourceCode.getTokenAfter(token);
            if (nextToken && nextToken.type === 'Punctuator' && nextToken.value === ':')
                return false;
        }
        const returnValue = PUNCTUATORS.has(value) || value.endsWith('=');
        return returnValue;
    }
