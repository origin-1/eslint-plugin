 ...
 * @param {string} [options.messageId] The ID of the message to report.
 * Defaults to `"unexpectedSpace"`.
 * @param {Object.<string, string>} [options.data] Data to format the message.
 * @param {boolean} [options.fixable]
 * Whether the problem can be fixed by removing the white space.
 * Defaults to `true`.
 * @returns {void}
 */
function reportOnSpace(firstToken, lastToken, context, node, options = { })
{
    ...
    if (match)
    {
        const { messageId = 'unexpectedSpace', data, fixable = true } = options;
        ...
        const report =
        {
            ...
            messageId,
            data,
            fix:        fixable ? fixer => fixer.removeRange(rangeToRemove) : null,
        };
